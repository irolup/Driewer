...
void main(void)
{
    ...
    for (int i=0; i<lightsNumber; i++){
        ...
        if (lambertTerm > 0.0)
        {
            final_color += lights.light[i].diffuse * material.diffuse * material.ambient * material.emission *  lambertTerm * tex01_color;
            
            vec3 H = normalize(L + V); // Calcul du vecteur de mi-distance
            
            float specular = pow( max(dot(N, H), 0.0), material.shininess); // Utilisation de H au lieu de R
