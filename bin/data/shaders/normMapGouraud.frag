...
void main(void)
{
    ...
    for (int i = 0; i < lightsNumber; i++)
    {
        ...
        // Calcul de la composante spéculaire selon le modèle de Phong
        vec3 R = reflect(-L, PN);
        float specAngle = max(dot(R, V), 0.0);
        float specularStrength = pow(specAngle, material.shininess);
        vec3 specular = (lights.light[i].specular.rgb * material.specular.rgb) * specularStrength;
        totalSpecular += specular;

        
        material_color += material.ambient * material.emission;

        
    }
    
    // Addition de la composante spéculaire à la couleur finale
    vec3 diffuse_specular = totalDiffuse + totalSpecular;
    
    fragColor = vec4(diffuse_specular, 1.0) * texture(texture_diffuse, uv) * material_color;
}
